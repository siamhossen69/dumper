...
def banner():
 ...
 .d8888b. 8888888        d8888 888b     d888 
d88P  Y88b  888         d88888 8888b   d8888 
Y88b.       888        d88P888 88888b.d88888 
 "Y888b.    888       d88P 888 888Y88888P888 
    "Y88b.  888      d88P  888 888 Y888P 888 
      "888  888     d88P   888 888  Y8P  888 
Y88b  d88P  888    d8888888888 888   "   888 
 "Y8888P" 8888888 d88P     888 888       888 
                                             """)
def cek_cookie():
	try:
...
def login():
	banner()
	x=f"{P2}halo pengguna script dump ASEP YUSUP :)\n{P2}silahkan Select fitur login cookie untuk melanjutkan ke menu dump multi fast.. klo tidak mengerti apaÂ² bisa ketik {M2}help {P2}untuk meminta bantuan !!"
	vprint(panel(x,style=f"{H3}"))
	print("")
	x=f"{P2}[01] login with cookie\n{P2}[02] report bug script\n{P2}[{M2}00{P2}] exit "
	vprint(panel(x,style=f"{H3}"))
	cukuf = input(f" {P}[{H}â€¢{P}] Select : {H}")
	if cukuf in ["help","Help","HELP"]:
		print("")
		x=f"{P2}Telegram admin *--> {H2}https://t.me/ASEPYUSUP {P2}harap chat klo ada kepentingan yang mau disampaikan ke author dmf\nini klo gak bisa diarahin ke telegram admin yakk"
		vprint(panel(x,style=f"{H3}"))
		print("")
		x=f"{P2}sedang diarahkan ke Telegram author"
		vprint(panel(x,style=f"{H3}"))
		os.system('xdg-open https://t.me/ASEPYUSUP?text=bang+cara+pake+script+abang+kek+mana?')
		input(f" {P}[{H}â€¢{P}] kembali")
		login()
	elif cukuf in ["1","01"]:
		login_cookie()
	elif cukuf in ["2","02"]:
		print("")
		x=f"{P2}Telegram admin *--> {H2}https://t.me/ASEPYUSUP {P2}harap chat klo memang ada yang error\nini klo gak bisa diarahin ke telegram admin yakk"
		vprint(panel(x,style=f"{H3}"))
		print("")
		x=f"{P2}sedang diarahkan ke Telegram author"
		vprint(panel(x,style=f"{H3}"))
		os.system('xdg-open https://t.me/ASEPYUSUP?text=bang+script+mu+itu+ada+yang+error!!')
		input(f" {P}[{H}â€¢{P}] kembali")
		login()
	elif cukuf in ["0","00"]:
		exit()
...
def login_cookie():
	...
	with requests.Session() as xyz:
		try:
			...
			x=f"{P2}TOKEN EAAG BUAT SHARE POSTING FB ðŸ‘‡ðŸ‘‡{P2}\n{token}"
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk ke menu ")
			menu()
		except requests.exceptions.ConnectionError:
...
def menu():
	...
	try:lok = x["locale"]
		...
		lok = M+"-"+P
	print("")
	x=f"{P2}This is not cracking script!!.. This is just a dumping script\n{H2}[-] Script Maker : Siam Hossen."
	vprint(panel(x,style=f"{H3}"))
	print("")
	x=f"{P2} â”Œâ”€ Welcome {K2}{nama}\n{P2} â”‚ â””â”€ Date of Birth: {H2}{pko}\n{P2} â”‚ â””â”€ ID user : {H2}{tumbal_id}\n{P2} â”‚ â””â”€ IP Address : {H2}{IP}\n{P2} â”‚ â”Œâ”€ Your Country : {H2}{nibba}\n{P2} â””â”€ Current Date : {H2}{sekarang}"
	vprint(panel(x,style=f"{H3}"))
	print("")
	x=f"{P2}[01] Dump from public\n{P2}[02] Dump from followers\n{P2}[03] Dump from post likes\n{P2}[04] Dump from public v2\n{P2}[05] Dump from followers v2\n{P2}[06] Check cracking result\n{P2}[09] Tracked ip\n{P2}[07] Share posting fb"
	vprint(panel(x,style=f"{H3}"))
	...
	if hikmat in ["1","01"]:
		...
		if f in ["1"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}1 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ")
			menu()
		elif f in ["2"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}2 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ")
			menu()
		elif f in ["3"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}3 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ")
			menu()
		elif f in ["4"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}4 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ") 
			menu()
		elif f in ["5"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}5 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ")
			menu()
		elif f in ["6"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}6 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ")
			menu()
		elif f in ["7"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}7 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ")
			menu()
		elif f in ["8"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}8 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ")
			menu()
		elif f in ["9"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}9 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ")
			menu()
		elif f in ["10"]:
			print("")
			x=f"{P2}terimakasih telah memberi bintang {H2}10 {P2}untuk script kami >< semoga saja kamu sehatÂ² selalu amin :) "
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter untuk kembali ")
			menu()
		else:
...
def dump_public():
	...
	try:
		...
		cuy = input(f"{garis} file save as :\x1b[38;5;46m ")
		print("")
		wkwk  = ('/sdcard/' + cuy + '.txt').replace(' ', '_')
		xxx = open(wkwk, 'w')
		token = open('token.txt','r').read()
		...
		for fuck in cyna['friends']['data']:
			...
			sys.stdout.write(f'\r\033[0m *--> %s {fr}â€¢ \033[0m%s                  \r\n\x1b[38;5;231m [ \x1b[38;5;46m%s\x1b[38;5;231m ] [ \x1b[38;5;46m%s\x1b[38;5;231m ] \x1b[0;96mProcessing...'%(fuck['id'],fuck['name'],datetime.datetime.now().strftime('%H:%M:%S'), len(id)
			)); sys.stdout.flush()
			time.sleep(0.0050)
			
		xxx.close()
		jalan(f"\n{garis} Public Id dumping Successful");print(f"{garis} file saved as *--> [ %s%s%s ]"%(H,wkwk,P))
		input(f"{garis} Press Enter ");menu()
	except (KeyError,IOError):
		os.remove(wkwk)
	...
	for uik in idl:
		try:
			...
			try:
				...
				continue
			x=f"{P2}â”Œâ”€ ID : {uik}\n{P2}â”‚ â””â”€ NAMA : {ciner['name']}\nâ”‚ â””â”€ LOKASI : {ciner['locale']}\nâ”‚ â”Œâ”€ LINK : {ciner['link']}\nâ””â”€ FRIENDS : {len(to)}"
